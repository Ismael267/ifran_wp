<?php
/* Export Referral Layout */

if ( !defined( 'ABSPATH' ) ) {
    exit ; // Exit if accessed directly.
}
?><h2><?php _e( 'Export Referrals' , FS_AFFILIATES_LOCALE ) ; ?></h2>
<form method="POST" class="fs_affiliates_export_referral_data_form">
    <table class="form-table">
        <tbody>
            ...
            <tr>
                <th><label><?php _e( 'Referral Status' , FS_AFFILIATES_LOCALE ) ?></label></th>
                <td>
                    <select name="fs_affiliates_status">
                        <option value="all"><?php _e( 'All Statuses' , FS_AFFILIATES_LOCALE ) ?></option>
                        <option value="fs_pending"><?php _e( 'Pending' , FS_AFFILIATES_LOCALE ) ?></option>
                        <option value="fs_unpaid"><?php _e( 'Unpaid' , FS_AFFILIATES_LOCALE ) ?></option>
                        <option value="fs_paid"><?php _e( 'Paid' , FS_AFFILIATES_LOCALE ) ?></option>
                        <option value="fs_rejected"><?php _e( 'Rejected' , FS_AFFILIATES_LOCALE ) ?></option>
                    </select>
                </td>
            </tr>
            <tr>
                <th><label><?php _e( 'Export Referrals' , FS_AFFILIATES_LOCALE ) ?></label></th>
                <td>
                    <input type="hidden" name="fs_nonce" value="<?php echo wp_create_nonce( 'export_referrals' ) ?>"/>
                    <input type="hidden" name="action" value="export_referrals"/>
                    <input type="submit" style="margin-top: 0px !important" class="fs_affiliates_export_data fs_affiliates_add_btn" value="<?php _e( 'Export Referrals as CSV' , FS_AFFILIATES_LOCALE ) ; ?>"/>
                </td>
            </tr>
        </tbody>
    </table>
