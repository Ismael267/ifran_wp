	...
	class Woostify_Filter_Render {
		...
		public function render_hierarchical( $filter_id, $output, $selected_value, $expand_default ) {
			...
			foreach ( $values as $key => $value ) {
				...
				$checked_input = ! empty( $selected_value ) && in_array( $checkbox_id, $selected_value, true ) ? 'checked' : '';
				?>
				<div class="w-filter-item-wrap">
					<label class="w-filter-item" for="<?php echo esc_attr( $checkbox_html_id ); ?>" data-id="<?php echo esc_attr( $checkbox_id ); ?>">
		...
		public function render_filter( $filter_id = false, $product_ids = array(), $selected_value = false, $data = array(), $empty = false, $is_tax = false ) {
			...
			if ( ! empty( $condition_term ) ) {
				if ( empty( $product_taxonomy_id ) ) {
					...
					if ( isset( $condition_term['include'] ) ) {
						$condition_term['include'] = array_unique( array_merge( $condition_term['include'], $product_taxonomy_id ) );
			...
			switch ( $type ) {
				...
				case 'checkbox':
					...
					foreach ( $output as $key => $check ) {
						...
						<div class="w-filter-item-wrap">
							<label class="w-filter-item" for="<?php echo esc_attr( $checkbox_html_id ); ?>" data-id="<?php echo esc_attr( $checkbox_id ); ?>">
								<input class="w-filter-item-input" id="<?php echo esc_attr( $checkbox_html_id ); ?>" <?php echo in_array( $checkbox_id, $checked_value, true ) ? 'checked' : ''; ?> type="checkbox" name="w-filter-checkbox-<?php echo esc_attr( $source ); ?>">
				...
				case 'radio':
					...
					$sql    = "SELECT DISTINCT term_name, term_id, COUNT(DISTINCT product_id) as count FROM $table_name WHERE term='$source' AND term <> '' AND term_id <> 0 $where_product_ids $where_term_ids GROUP BY term_id ORDER BY $orderby";
					$output = $wpdb->get_results($sql, ARRAY_A); // phpcs:ignore
					if ( empty( $output ) ) {
						break;
					}
					...
					// Load data from url.
					$radio_selected_value = empty( $selected_value ) ? (int) $get_url_params : (int) $selected_value;
					...
					foreach ( $output as $key => $radio ) {
						...
						?>
						<div class="w-filter-item-wrap">
							<label class="w-filter-item" for="<?php echo esc_attr( $radio_html_id ); ?>" data-id="<?php echo esc_attr( $radio_id ); ?>">
								<input class="w-filter-item-input" id="<?php echo esc_attr( $radio_html_id ); ?>" <?php checked( $radio_selected_value, $radio_id ); ?> type="radio" name="w-filter-radio-<?php echo esc_attr( $source ); ?>">
				...
				case 'select':
					...
					// Load data from url.
					$_selected_value = empty( $selected_value ) ? (int) $get_url_params : (int) $selected_value;
					?>
					...
					foreach ( $output as $key => $select ) {
						...
						?>
							<option value="<?php echo esc_attr( $select['term_id'] ); ?>" <?php selected( $_selected_value, (int) $select['term_id'], true ); ?>>
							<?php
							if ( $item_count ) {
