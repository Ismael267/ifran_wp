	...
	class Woostify_Product_Filter {
		...
		public function get_active_params() {
			...
			foreach ( $get_params as $key => $value ) {
				...
				if ( in_array( $type, array( 'checkbox', 'visual' ), true ) ) {
					...
					case 'visual':
						...
						if ( is_array( $v ) ) {
							foreach ( $v as $vv ) {
								...
								$filter_key .= '<span class="w-filter-key-remove" data-id="' . $k . '" data-type="' . $type . '" data-value="' . $vv . '">' . sprintf( /* translators: taxonomy name, value */__( '%1$s: %2$s', 'woostify-pro' ), $label, $get_terms->name ) . '<span class="w-filter-key-remove-icon">' . $remove_icon . '</span></span>';
							}
						} else {
							...
							$filter_key .= '<span class="w-filter-key-remove" data-id="' . $k . '" data-type="' . $type . '" data-value="' . $v . '">' . sprintf( /* translators: taxonomy name, value*/__( '%1$s: %2$s', 'woostify-pro' ), $label, $get_term->name ) . '<span class="w-filter-key-remove-icon">' . $remove_icon . '</span></span>';
						}
						break;
				}
			}
						...
						<td>
							<div class="w-filter-container">
								<div class="w-filter-field">
									<select name="woostify_product_filter_data" class="<?php echo esc_attr( 'visual' === $filter_type ? 'product-attr-only' : '' ); ?>">
